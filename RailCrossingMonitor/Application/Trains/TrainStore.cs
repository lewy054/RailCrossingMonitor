...
public sealed class TrainStore
{
    ...
    public void Upsert(IEnumerable<PortalTrainDto> trains)
    {
        ...
        foreach (var train in trains)
        {
            ...
            /*
             * Portal czasami wysyła kilka razy dokładnie tę samą
             * pozycję GPS.
             *
             * Nie traktujemy takiej aktualizacji jako nowego
             * pomiaru pozycji.
             *
             * Dzięki temu ReceivedAtUtc pozostaje czasem ostatniej
             * rzeczywiście zmienionej pozycji.
             */
            var positionChanged = distanceKm > 0.005;

            double? speedKmh = previous.SpeedKmh;
            double? headingDegrees = previous.HeadingDegrees;
            DateTimeOffset positionReceivedAtUtc =
                previous.ReceivedAtUtc;

            if (positionChanged)
            {
                var seconds =
                    (receivedAt - previous.ReceivedAtUtc)
                    .TotalSeconds;
                ...
                /*
                 * Tylko rzeczywiście nowa pozycja GPS dostaje
                 * nowy czas.
                 */
                positionReceivedAtUtc = receivedAt;
            }

            /*
             * Nadal aktualizujemy surowe dane z Portalu:
             * Status, Info, Carrier, Number itd.
             *
             * Jeżeli pozycja się nie zmieniła, pozostawiamy:
             * - poprzedni ReceivedAtUtc,
             * - poprzednią prędkość,
             * - poprzedni kierunek.
             */
            _trains[train.t] = new TrainPosition(
                ...
                Code: train.c,
                Angle: train.a,
                ReceivedAtUtc: positionReceivedAtUtc,
                SpeedKmh: speedKmh,
